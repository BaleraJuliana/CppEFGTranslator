...
te::sa::ProximityMatrixCreatorDialog::~ProximityMatrixCreatorDialog() = default;
...
void te::sa::ProximityMatrixCreatorDialog::onOkPushButtonClicked()
{
  ...
  te::sa::GPMConstructorAbstractStrategy* constructor = nullptr;
  ...
  te::sa::GPMWeightsAbstractStrategy* weights = nullptr;
  ...
  te::sa::GeneralizedProximityMatrix* gpm;
  ...
  if(!m_ui->m_dsRadioButton->isChecked())
  {
    ...
    if(m_ui->m_galRadioButton->isChecked())
      exchanger .exportToGAL(gpm, fullFileName);
    else if(m_ui->m_gwtRadioButton->isChecked())
      exchanger .exportToGWT(gpm, fullFileName, 1); // use 1 pos to export the weight information
  }
