...
//connectors
  ...
  connect(m_ui->m_outputSRIDToolButton, SIGNAL(clicked()), this, SLOT(onOutputLayerSRSTriggered()));
...
te::qt::widgets::DirectExchangerDialog::~DirectExchangerDialog() = default;
...
bool te::qt::widgets::DirectExchangerDialog::exchangeToFile()
{
  ...
  try
  {
    ...
    QFileInfo fileInfo(m_ui->m_dataSetLineEdit->text());

    //create data source
    std::string connInfo("file://" + std::string(fileInfo.absoluteFilePath().toUtf8().data()));

    if (fileInfo.suffix().toUtf8().data() == std::string("csv"))
      connInfo += "?GEOMETRY=AS_WKT";
    ...
    std::unique_ptr<te::da::DataSetAdapter> dsAdapter(te::da::CreateAdapter(dataset.get(), converter));
...
bool te::qt::widgets::DirectExchangerDialog::exchangeToDatabase()
{
  ...
  std::unique_ptr<te::da::DataSourceTransactor> transactor;
...
void te::qt::widgets::DirectExchangerDialog::onDirToolButtonClicked()
{
  QString fileName = QFileDialog::getSaveFileName(this, tr("Save as..."),
    QString(), tr("Shapefile (*.shp *.SHP);;Mapinfo File (*.mif *.MIF);;KML (*.kml *.KML);;GeoJSON (*.geojson *.GEOJSON);;GML (*.gml *.GML);;DXF (*.dxf *.DXF);;DGN (*.dgn *.DGN);; CSV (*.csv *.CSV);;"), nullptr, QFileDialog::DontConfirmOverwrite);
