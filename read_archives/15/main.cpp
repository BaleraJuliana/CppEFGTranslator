    ...
    m_ui(new Ui::RasterInfoWidgetForm)
{
  ...
  connect(m_ui->m_extraParametersCheckBox, SIGNAL(clicked(bool)), this, SLOT(onExtraParametersCheckBoxChecked(bool)));
}

te::qt::widgets::RasterInfoWidget::~RasterInfoWidget() = default;
...
std::map<std::string, std::string> te::qt::widgets::RasterInfoWidget::getInfo(int count, std::string name) const
{
  std::map<std::string, std::string> rinfo;
...
{
  m_ui->m_rasterPropertiesFrame->setVisible(false);
}
...
void te::qt::widgets::RasterInfoWidget::onOpenFileDlgToolButtonClicked()
{
  ...
  if( m_outputMode )
  {
    te::qt::widgets::FileDialog fileDialog(this, te::qt::widgets::FileDialog::RASTER, "rp_raster_info");
    ...
    m_originalFullFileName = fileDialog.getPath();
  }
  else
  {
    m_originalFullFileName = QFileDialog::getOpenFileName(this, tr("Select File"), 
      ...
      te::qt::widgets::GetDiskRasterFileSelFilter( false ), nullptr ,QFileDialog::ReadOnly).toUtf8().data();
  }
...
void te::qt::widgets::RasterInfoWidget::onOpenSRIDDlgToolButtonClicked()
{
  ...
  te::qt::widgets::SRSManagerDialog diag( this, nullptr );
  diag.exec();
