...
te::sa::SkaterDialog::~SkaterDialog() = default;
...
//get selected layer
  ...
  std::unique_ptr<te::da::DataSetType> dataSetType = l->getSchema();
  std::unique_ptr<te::da::DataSet> dataSet = l->getData();
...
//create datasource to save the output information
  ...
  inParams->m_ds = std::move(dataSet);
  inParams->m_dsType = std::move(dataSetType);
  inParams->m_gpmAttrLink = gpm->getAttributeName();
  inParams->m_gpm = std::move(gpm);
  ...
  if(m_ui->m_clustersCheckBox->isChecked() && m_ui->m_popCheckBox->isChecked())
  {
    ...
    inParams->m_attrPop = m_ui->m_popComboBox->currentText().toUtf8().data();
  }
...
void te::sa::SkaterDialog::onTargetFileToolButtonPressed()
{
  ...
  QString fileName = QFileDialog::getSaveFileName(this, tr("Save as..."), QString(), tr("Shapefile (*.shp *.SHP);;"),nullptr, QFileDialog::DontConfirmOverwrite);
...
std::unique_ptr<te::sa::GeneralizedProximityMatrix> te::sa::SkaterDialog::loadGPM()
{
  std::unique_ptr<te::sa::GeneralizedProximityMatrix> gpm;
  ...
  if(!m_ui->m_gpmGroupBox->isChecked())
  {
    ...
    //get attrlink
    std::unique_ptr<te::da::DataSetType> dsType = dsLayer->getSchema();
