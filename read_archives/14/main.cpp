...
//connects
 ...
 te::qt::widgets::MultiResolutionWidget::~MultiResolutionWidget() = default;

 Ui::MultiResolutionWidgetForm*
 te::qt::widgets::MultiResolutionWidget::getForm() const
 {
   return m_ui.get();
}
...
bool te::qt::widgets::MultiResolutionWidget::fromLayer()
{
  ...
  std::unique_ptr<te::rst::Raster> rst = ds->getRaster(rpos);
...
bool te::qt::widgets::MultiResolutionWidget::fromFile()
{
  ...
  std::unique_ptr<te::rst::Raster> rst(te::rst::RasterFactory::open(rinfo));
...
bool te::qt::widgets::MultiResolutionWidget::fromFolder()
{
  ...
  {
    std::unique_ptr<te::da::DataSet> dataSet = ds->getDataSet(rasterNames[t]);

    if(!dataSet.get())
      continue;

    std::size_t rpos = te::da::GetFirstPropertyPos(dataSet.get(), te::dt::RASTER_TYPE);
    std::unique_ptr<te::rst::Raster> rst = dataSet->getRaster(rpos);
