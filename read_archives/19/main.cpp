...
#include "ui/ui_ImageGenerateDialogForm.h"
  ...
  m_inDataSource(nullptr)
{
  ...
  m_ui->m_HelpPushButton->setPageReference("plugins/mnt/DTM_Shadow.html");
...
te::mnt::ImageGenerationDialog::~ImageGenerationDialog() = default;
    ...
    if (it->get())
    {
      if (it->get()->isValid())
      {
        ...
        if (dsType.get())
        {
          ...
          if (type == GRID)
          {
            std::unique_ptr<te::da::DataSet> inds(it->get()->getData());
...
void te::mnt::ImageGenerationDialog::onTargetFileToolButtonPressed()
{
  ...
  QString fileName = QFileDialog::getSaveFileName(this, tr("Select the output file name"),
    ...
    te::qt::widgets::GetDiskRasterFileSelFilter(true), nullptr, nullptr).toUtf8().data();
...
void te::mnt::ImageGenerationDialog::onOkPushButtonClicked()
{
  try
  {
    ...
    te::qt::widgets::applyRasterMultiResolution(tr("Image Generation"), rst.get());
    ...
    {
      double min, max;
      te::mnt::getMinMax(rst.get(), min, max);
...
void te::mnt::ImageGenerationDialog::drawPreview(std::unique_ptr<te::rst::Raster>& raster)
{
  ...
  {
    // Draw raster
    te::map::RasterContrast contrast(te::map::RasterTransform::LINEAR_CONTRAST, 1);
    double min, max;
    te::mnt::getMinMax(outrst, min, max);
    double gain, offset1, offset2;
    if (te::rp::Contrast::getGainAndOffset(te::rp::Contrast::InputParameters::LinearContrastT, min, max, 0., 255., gain, offset1, offset2))
    {
      contrast.setValues(gain, offset1, offset2, min, max, 0);
    }
    bool cancel = false;
    te::map::DrawRaster(outrst, m_canvas, *envRst, m_mapDisplay->getSRID(), *envRst, m_outsrid, cs, &contrast, m_mapDisplay->getScale(), &cancel);
